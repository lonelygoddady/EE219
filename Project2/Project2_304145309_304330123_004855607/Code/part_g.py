...
if __name__ == "__main__":
    ...
    clf = MultinomialNB()
    bayes_model = clf.fit(X_train_tfidf.toarray(), twenty_train.target) # train the data over high-dimension data and known Y values

    Y_predict = bayes_model.predict(X_test_tfidf) # predict the value with the learning algorithm

    # Calculating the accuracy, recall, precision and confusion matrix
    accuracy_bayes = np.mean(twenty_test.target == Y_predict)
    print ('The accuracy for the model is %f' % accuracy_bayes)
    ...
    # Plotting the ROC
    if not os.path.exists('../Graphs/part_G'):
        os.makedirs('../Graphs/part_G')
    probas_ = bayes_model.predict_proba(X_test_tfidf)
    fpr, tpr, thresholds = metrics.roc_curve(twenty_test.target, probas_[:, 1])
    plt.figure(7)
    plt.plot(fpr, tpr, lw=1, label = "Multinomial Naive Bayes ROC")
    plt.xlim([-0.05, 1.05])
    plt.ylim([-0.05, 1.05])
