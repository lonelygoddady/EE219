...
def part_e():
    ...
    twenty_train, x_tfidf, x_counts, count_vect = help_functions.TFIDF(categories, 'all', stop_words, NLTK_StopWords)
    ...
    nmf = NMF(n_components=k_value, random_state=random_state,
            max_iter=n_iter, init='nndsvda', solver='cd').fit(x_tfidf)
    ...
    #                                       np.max(x_tfidf_pca) - np.min(x_tfidf_pca))
    # x_tfidf_nmf = help_functions.logrithm(x_tfidf_nmf,
    #                                       np.max(x_tfidf_nmf) - np.min(x_tfidf_nmf))
    #
    # help_functions.bench_k_means(KMeans(init='k-means++', n_clusters=n_clusters, n_init=10),
    ...
    #                                            np.max(x_tfidf_pca_norm) - np.min(x_tfidf_pca_norm))
    # x_tfidf_nmf_norm = help_functions.logrithm(x_tfidf_nmf_norm,
    #                                            np.max(x_tfidf_nmf_norm) - np.min(x_tfidf_nmf_norm))
    # #
    # help_functions.bench_k_means(KMeans(init='k-means++', n_clusters=n_clusters, n_init=10),
