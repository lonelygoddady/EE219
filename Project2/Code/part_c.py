...
if __name__ == "__main__":
	...
	features = count_vect.get_feature_names() #get the term names from the term vector

	#extract 10 most sigificant terms from following class 
	target_cat_index = [twenty_train.target_names.index("comp.sys.ibm.pc.hardware"),twenty_train.target_names.index("comp.sys.mac.hardware"),\
	    twenty_train.target_names.index("misc.forsale"), twenty_train.target_names.index("soc.religion.christian")]

	for x in range(len(target_cat_index)):
	    row = tf_icf_final[x].toarray()[0]  #get all term values in tf_icf matrix for that category 
	    zipped_row = zip(row,features) #combine term frequency and names
	    sort_row = sorted(zipped_row)
	    print 'The 10 most significant terms in ' + str(twenty_train.target_names[x]) + ' is' \
	    + str(zip(*sort_row[-9:])[1])
